...
def detect_retina(img_vol, pixelsX=200, numFrames=200, pixelsZ=1024,
                  xWidth=6, yWidth=6, zWidth=2,
                  normalize=True,
    ...
    # How many microns does each pixel cover? (microns/pixel)
    # An area of 6*6*2 mm^3 captures in a 200*200*1024 pixels data cube.
    # 6mm = 6000 microns => 6000/200 = 30 microns per pixel.
    # 2mm = 2000 microns => 2000/200 = 10 microns per pixel.
    ...
    # Locations of maximum gradients will be inspected DISTCONST apart.
    #   For Spectralis, set to 96.68 (~97) - Default value
    params['distconst'] = 48.828125
    ...
    #   Example:
    #       STD of the Gaussian kernel for filtering is 33.34 microns
    #       By dividing it to xScale, we can get 1.1 pixels.
    #       So, a Gaussian kernel with std of 1.1 pixels is used to smooth the image in the
    #       specific direction.
    ...
    # Gaussian denoising
    #   Input volume (Z*X*Y) is denoised in two directions:
    #       1. Axial direction (Z) using a filter with std = sigma_ax
    #       2. Lateral direction (X) with std = sigma_lat
    #   During filtering of the above directions, always Y direction is also filtered.
    ...
    # Find the two largest gradient corresponds to either ILM or IS/OS
    #   1. find the locations of maximum gradient
    #
    #   2. inspect the locations 25 pixels (~97 microns) apart to find the
    #   two biggest gradients.

    # Find maximum gradient locations
    #   max1pos.shape => (200,200)
    #   Given all A-scans in all B-scans, "max1pos" stores the location
    #   of the largest gradient for each A-scan.
    grad_o = grad.copy()
